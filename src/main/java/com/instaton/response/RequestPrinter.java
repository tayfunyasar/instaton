...
public class RequestPrinter {
	 ...
	 *            Request parameter.
	 * @param printSession
	 *            Enable session information printing
	...
	private static String debugStringCookie(Cookie cookie, String indentString) {
		if (cookie == null) {
			...
			sb.append(indentString).append(INDENT_UNIT).append("'name': '").append(cookie.getName()).append(NEWLINE_SEPERATOR);
			sb.append(indentString).append(INDENT_UNIT).append("'value': '").append(cookie.getValue()).append(NEWLINE_SEPERATOR);
			sb.append(indentString).append(INDENT_UNIT).append("'domain': '").append(cookie.getDomain()).append(NEWLINE_SEPERATOR);
			sb.append(indentString).append(INDENT_UNIT).append("'path': '").append(cookie.getPath()).append(NEWLINE_SEPERATOR);
			sb.append(indentString).append(INDENT_UNIT).append("'max_age': ").append(cookie.getMaxAge()).append(NEWLINE_SEPERATOR);
			sb.append(indentString).append(INDENT_UNIT).append("'version': ").append(cookie.getVersion()).append(NEWLINE_SEPERATOR);
			sb.append(indentString).append(INDENT_UNIT).append("'comment': '").append(cookie.getComment()).append(NEWLINE_SEPERATOR);
			sb.append(indentString).append(INDENT_UNIT).append("'secure': '").append(cookie.getSecure()).append(NEWLINE_SEPERATOR);
			sb.append(indentString).append("}");
			return sb.toString();
		}
	}
	...
	private static String debugStringCookies(HttpServletRequest request, int indent) {
		...
		for (Cookie cookie : request.getCookies()) {
			sb.append(RequestPrinter.debugStringCookie(cookie, indentString + INDENT_UNIT)).append(NEWLINE_SEPERATOR);
			cookieCount++;
		}
		if (cookieCount > 0) {
			sb.delete(sb.length() - ",\n".length(), sb.length());
	...
	private static String debugStringHeader(String indentString, String headerName, List<String> headerValues) {
		StringBuilder sb = new StringBuilder();
		sb.append(indentString).append(INDENT_UNIT).append("'").append(headerName).append("': ");
	...
	private static String debugStringHeaders(HttpServletRequest request, int indent) {
		...
		while (headerNames.hasMoreElements()) {
			...
			sb.append(RequestPrinter.debugStringHeader(indentString, headerName, headerValuesList)).append(",\n");
		}
		sb.append(indentString).append("}\n");
		return sb.toString();
	}
	...
	private static String debugStringParameter(String indentString, String parameterName, String[] parameterValues) {
		StringBuilder sb = new StringBuilder();
		sb.append(indentString).append(INDENT_UNIT).append("'").append(parameterName).append("': ");
	...
	private static String debugStringParameters(HttpServletRequest request, int indent) {
		...
		while (parameterNames.hasMoreElements()) {
			...
			sb.append(RequestPrinter.debugStringParameter(indentString, parameterName, parameterValues)).append(",\n");
		}
		sb.append(indentString).append("}\n");
		return sb.toString();
	}
	...
	private static String debugStringSession(HttpSession session, int indent) {
		...
		StringBuilder sb = new StringBuilder();
		sb.append(indentString).append("{\n");
		sb.append(indentString).append(INDENT_UNIT).append("'id': '").append(session.getId()).append(NEWLINE_SEPERATOR);
		...
		sb.append(indentString).append(INDENT_UNIT).append("'is_new': '").append(session.isNew()).append(NEWLINE_SEPERATOR);
		...
		while (attributeNames.hasMoreElements()) {
			...
			sb.append(indentString).append(INDENT_UNIT).append("'").append(attributeName).append("': ").append("'").append(o.toString()).append("',\n");
		}
