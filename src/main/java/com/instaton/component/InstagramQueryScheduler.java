    ...
    private void start() {
      ...
      final String query =
          this.searchQueryEntity.getQuery()
              + " "
              + InstagramQueryScheduler.this.webConstants.getExcludedQuery();

      final SearchParameters searchParameters =
          new SearchParameters(query).count(100).resultType(ResultType.RECENT);
      final SearchResults search =
          InstagramQueryScheduler.this.twitterService.getSearch(searchParameters);
      final List<Tweet> filterTweets =
          InstagramQueryScheduler.this.twitterService.filterTweets(search.getTweets());
      ...
      for (final Tweet tweet : filterTweets) {
        ...
        InstagramQueryScheduler.this.twitterUserService.save(user);
      }
      InstagramQueryScheduler.this.twitterUserService.evictfindAll();
