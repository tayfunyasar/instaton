...
public class BaseRequestUtils {
  ...
  public static String getSSOTokenFromRequest(
      String SsoTokenReqParamName, HttpServletRequest request) {
    ...
    if (refererHost != null) {
      for (String domainName : allowedDomainList) {
        ...
        if (StringUtils.equalsIgnoreCase(refererHost, domainName)) {
          logger.info(
              "refererHost = ( {} ) equalsIgnoreCase domainName = ( {} )", refererHost, domainName);
          return true;
        }
      }
    } else {
