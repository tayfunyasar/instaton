...
public class TwitterSearchController {
	...
	public static String detechText(final String s) throws IOException {
		...
		final LanguageDetector languageDetector = LanguageDetectorBuilder.create(NgramExtractors.standard()).withProfiles(languageProfiles).build();
	...
	@Autowired
	private BlackNameEntityService blackNameEntityService;
	...
	public CustomSearchResults getCurrent() throws IOException {
		...
		final String excludedKeywords = " -izmir -adana -mersin -trabzon -haber -spor -radyo -galatasaray -fenerbahçe -besiktas -gazete AND -filter:retweets AND -filter:replies";
		final SearchParameters param1 = new SearchParameters(excludedKeywords).geoCode(geoCode).lang("tr").count(100);
		params.add(param1);
		final SearchParameters param2 = new SearchParameters(excludedKeywords).lang("tr").count(100).includeEntities(true).resultType(ResultType.RECENT);
		params.add(param2);

		final ArrayList<Tweet> tweets = new ArrayList<>();
		for (final String term : terms) {
			final SearchParameters paramTerm = new SearchParameters(term + excludedKeywords).count(100).includeEntities(true).resultType(ResultType.RECENT);
		...
		for (final Tweet tweet : search.getTweets()) {
			...
			for (final BlackNameEntity blackNameEntity : blackNameEntityList) {
				if (StringUtils.startsWithIgnoreCase(TwitterSearchController.replaceAllTurkishCharacters(name), TwitterSearchController.replaceAllTurkishCharacters(blackNameEntity.getName()))) {
					isContains = true;
					System.out.println("blackNameEntity startsWithIgnoreCase = tweet.getUser().getName() " + name + " == blackNameEntity.getName()) = " + blackNameEntity.getName());
				}
				if (!isContains && StringUtils.containsIgnoreCase(TwitterSearchController.replaceAllTurkishCharacters(name), TwitterSearchController.replaceAllTurkishCharacters(blackNameEntity.getName()))) {
					isContains = true;
					System.out.println("blackNameEntity containsIgnoreCase = tweet.getUser().getName() " + name + " == blackNameEntity.getName()) = " + blackNameEntity.getName());
				}
			}
